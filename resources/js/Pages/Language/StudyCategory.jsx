...
import {
    ...
    CheckCircle2, TrendingUp, Play, Zap,
} from 'lucide-react';
                        ...
                        <tr key={ri} className="border-b border-white/5 last:border-0">
                            ...
                            {row.cells.map((cell, ci) => (
                                <td key={ci} className="px-2 py-3 text-lg font-bold text-white/80">
                                    {cell === '—' ? <span className="text-white/20">—</span> : <span>{cell}</span>}
                                </td>
                            ))}
                        </tr>
                    ))}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {pairs.map(([a, b], i) => (
                <div key={i} className="flex gap-1">
                    <span className="flex-1 text-center px-2 py-2 rounded-lg bg-white/5 text-sm font-medium">{a}</span>
                    {b && (
                        <>
                            <span className="text-gray-600 self-center">→</span>
                            <span className="flex-1 text-center px-2 py-2 rounded-lg bg-brand-500/10 border border-brand-500/20 text-sm font-medium text-brand-200">{b}</span>
                        </>
                    )}
                </div>
            ))}
...
function MediaBlock({ media }) {
    ...
    return (
        <div className="space-y-6">
            {media.map((block, i) => {
                if (block.type === 'kana-grid') return (
                    <Card key={i} className="p-5">
                        <h3 className="text-sm font-bold text-gray-300 mb-4 uppercase tracking-wider">{block.title}</h3>
                        <KanaGrid chart={block} />
                    </Card>
                );
                if (block.type === 'pair-grid') return (
                    <Card key={i} className="p-5">
                        <h3 className="text-sm font-bold text-gray-300 mb-4 uppercase tracking-wider">{block.title}</h3>
...
export default function StudyCategory({ languageId, categoryId }) {
    ...
    const overviewLesson = useMemo(() => {
        for (const bucket of curriculum) {
            const found = bucket.lessons.find(
                (l) => l.id === categoryId || l.id.includes(categoryId) || categoryId.includes(l.id)
            );
    ...
    return (
        <DashboardLayout>
            <Head title={`${categoryMeta.name} — ${config.name} ${t('sections.study')}`} />
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                    ...
                    <div className="text-right shrink-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                            {t('language.lessonsCount', { completed: completedCount, total: lessons.length })}
                        </p>
                    </div>
                </div>
            ...
            <section className="mb-10">
                ...
                {lessons.length === 0 ? (
                    <Card className="p-10 text-center border-dashed border-white/20">
                        ...
                        <p className="text-gray-400">{t('study.lessonsCategoryComingSoon')}</p>
                    </Card>
                ) : (
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {lessons.map((lesson, index) => {
                            ...
                            const diffLabel  = difficultyKey[lesson.difficulty]
                                ? t(`levels.${difficultyKey[lesson.difficulty]}`)
                                : lesson.difficulty;
                                ...
                                <Card className={cn(
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                                                <span className="inline-flex items-center gap-1">
                                                    <Signal className="w-3 h-3" />{diffLabel}
                                                </span>
                                                <span className="inline-flex items-center gap-1">
                                        ...
                                        <div className="ml-2 shrink-0">
                                            ...
                                            : isUnlocked ? <ChevronRight className="w-5 h-5 text-brand-300" />
                                    ...
                                    <div className="flex items-center justify-between mt-3">
                                        <span className={cn(
                                            ...
                                            isDone
                                                ...
                                                : isUnlocked
                                                ? (difficultyColorClass[lesson.difficulty] ?? 'text-brand-300 bg-brand-500/10 border-brand-500/20')
                                                : 'text-gray-400 bg-white/5 border-white/10',
                                        )}>
                                            {isDone ? t('stats.completed') : isUnlocked ? diffLabel : t('stats.locked')}
                                        </span>

                                        {isUnlocked && (
                                            <Button variant={isDone ? 'outline' : 'primary'} size="sm" className="gap-1 pointer-events-none">
                                                <Play className="w-3.5 h-3.5" />
                                                {isDone ? t('buttons.review') : t('buttons.start')}
                                            </Button>
                                        )}
                                    </div>
                                </Card>
                            ...
                            return isUnlocked
                                ? <Link key={lesson.id} href={href} className="block h-full">{card}</Link>
            ...
            {overviewLesson && (
                <section className="mb-10 space-y-6">
                        ...
                        <Card className="p-6">
                            ...
                            <div className="space-y-3">
                                {overviewLesson.examples.map((ex, i) => (
                                    <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-2 p-3 rounded-xl bg-white/5 border border-white/5">
                                        <span className="text-2xl font-bold text-white min-w-[6rem]">{ex.jp}</span>
                                        <span className="text-sm text-gray-400 italic min-w-[8rem]">{ex.reading}</span>
                                        <span className="text-sm text-gray-300 flex-1">{ex.en}</span>
                                    </div>
                                ))}
                            </div>
                        </Card>
                    ...
                    {overviewLesson.media?.length > 0 && <MediaBlock media={overviewLesson.media} />}
                </section>
            )}
            ...
            {practiceMeta && (
                ...
                >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 className="font-bold text-lg mb-1">{t('study.readyToTest')}</h3>
                            <p className="text-sm text-gray-400">
                                {t('language.questionsXP', {
                                    count: practiceMeta.questions,
                                    xp: practiceMeta.xpReward,
                                    difficulty: practiceMeta.difficulty,
                                })}
                            </p>
                        </div>
                        <Link href={`/languages/${languageId}/practice/${categoryId}`}>
                            <Button variant="primary" className="gap-2 whitespace-nowrap">
                                <Zap className="w-4 h-4" />{t('buttons.goToPractice')}
                            </Button>
                        </Link>
                    </div>
                </Card>
