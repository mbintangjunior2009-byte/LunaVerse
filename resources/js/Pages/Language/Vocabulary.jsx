...
export default function LanguageVocabulary({ languageId }) {
    ...
    const filtered = allWords.filter((word) => {
        ...
        const matchesCategory = activeCategory === 'all' || word.categoryId === activeCategory;
    ...
    return (
        <DashboardLayout>
            <Head title={`${config.name} - ${t('sections.vocabulary')}`} />
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-400 mt-2">{t('language.vocabDesc', { name: config.name })}</p>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 flex items-center gap-1">
                        <BookOpen className="w-3.5 h-3.5 text-brand-300" /> {t('stats.wordsLearned')}
                    </p>
                    <p className="text-3xl font-bold">{estimatedVocab.toLocaleString()}</p>
                </Card>
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 flex items-center gap-1">
                        <Star className="w-3.5 h-3.5 text-yellow-300" /> {t('stats.totalInLibrary')}
                    </p>
                    <p className="text-3xl font-bold">{allWords.length || '—'}</p>
                </Card>
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 flex items-center gap-1">
                        <Brain className="w-3.5 h-3.5 text-brand-300" /> {t('stats.lessonsCompleted')}
                    </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        placeholder={t('language.searchPlaceholder', { name: config.name })}
            ...
            {allWords.length === 0 ? (
                <Card className="p-12 text-center border-dashed border-white/20">
                    ...
                    <h2 className="text-xl font-bold mb-2">{t('language.vocabComingSoonTitle')}</h2>
                    <p className="text-gray-400 mb-6">{t('language.vocabComingSoonDesc', { name: config.name })}</p>
                    <Link
                    ...
                    >
                        <BookOpen className="w-4 h-4" /> {t('buttons.goToStudy')}
                    </Link>
                </Card>
            ) : filtered.length === 0 ? (
                <Card className="p-10 text-center border-dashed border-white/20">
                    <p className="text-gray-400">{t('language.noVocabMatch')}</p>
                </Card>
            ) : (
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((word, index) => {
                        ...
                        return (
                            ...
                            >
                                <Card className={cn('p-5 border transition-all',
                                    isUnlocked ? 'border-white/10 hover:border-brand-500/30' : 'border-white/5 opacity-60'
                                )}>
                                    {!isUnlocked && (
                                        <div className="flex items-center gap-1 text-xs text-gray-500 mb-2">
                                            <Lock className="w-3 h-3" /> {t('stats.locked')}
                                        </div>
                                    )}
                                    <div className="flex items-start justify-between mb-2">
                                        <span className="text-2xl font-bold" style={{ color: isUnlocked ? config.themeColor : undefined }}>
                                            {word.term}
                                        </span>
                                        {word.reading && <span className="text-xs text-gray-400 mt-1 ml-2">{word.reading}</span>}
                                    </div>
                                    <p className="text-sm text-gray-300 mb-3">{word.meaning}</p>
                                    {word.example && (
                                        <p className="text-xs text-gray-500 italic border-t border-white/5 pt-2 mt-2">{word.example}</p>
                                    )}
                                    <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                                        <span className="px-2 py-0.5 rounded-full border" style={{
                                            borderColor: `${config.themeColor}33`, color: config.themeColor,
