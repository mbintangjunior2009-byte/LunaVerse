...
export default function LanguageHub({ languageId }) {
    ...
    const currentLevelIndex = config.levels.findIndex((_, index) => {
    ...
    return (
        <DashboardLayout>
            <Head title={`${config.name} - ${t('language.hub')}`} />
                    ...
                    <div className="flex flex-wrap gap-6 mt-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-300">
                                {t('language.levelLabel')} <span className="text-white font-bold">{currentLevel.name}</span>
                            </span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Award className="w-5 h-5 text-brand-300" />
                            <span className="text-gray-300">
                                {config.certification.name}: <span className="text-white font-bold">{config.certification.currentLevel}</span>
                            </span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Zap className="w-5 h-5 text-brand-300" />
                            <span className="text-gray-300">
                                {t('language.xpLabel')} <span className="text-white font-bold">{progress.xp.toLocaleString()}</span>
                            </span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Flame className="w-5 h-5 text-orange-300" />
                            <span className="text-gray-300">
                                {t('language.streakLabel')} <span className="text-white font-bold">{t('language.streakValue', { count: 5 })}</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <Link href={`/languages/${languageId}/study`}>
                    <Card className="p-5 hover:border-brand-500/50 transition-colors cursor-pointer">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-bold">{t('sections.study')}</span>
                        </div>
                        <p className="text-sm text-gray-400">{t('language.continueLabel')}</p>
                    </Card>
                </Link>
                ...
                <Link href={`/languages/${languageId}/practice`}>
                    <Card className="p-5 hover:border-brand-500/50 transition-colors cursor-pointer">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-bold">{t('sections.practice')}</span>
                        </div>
                        <p className="text-sm text-gray-400">{t('language.testSkillsLabel')}</p>
                    </Card>
                </Link>
                ...
                <Link href={`/languages/${languageId}/progress`}>
                    <Card className="p-5 hover:border-brand-500/50 transition-colors cursor-pointer">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-bold">{t('sections.progress')}</span>
                        </div>
                        <p className="text-sm text-gray-400">{t('language.viewStatsLabel')}</p>
                    </Card>
                </Link>
                ...
                <Link href={`/languages/${languageId}/achievements`}>
                    <Card className="p-5 hover:border-brand-500/50 transition-colors cursor-pointer">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-bold">{t('sections.achievements')}</span>
                        </div>
                        <p className="text-sm text-gray-400">{t('language.viewBadgesLabel')}</p>
                    </Card>
                </Link>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 inline-flex items-center gap-1">
                        <BookOpen className="w-3.5 h-3.5 text-brand-300" /> {t('stats.overallProgress')}
                    </p>
                    <p className="text-3xl font-bold">{overallProgress}%</p>
                    <div className="mt-2 w-full h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                            className="h-full rounded-full transition-all duration-300"
                            style={{ width: `${overallProgress}%`, backgroundColor: config.themeColor }}
                        />
                    </div>
                </Card>
                ...
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 inline-flex items-center gap-1">
                        <Play className="w-3.5 h-3.5 text-brand-300" /> {t('stats.lessonsCompleted')}
                    </p>
                ...
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 inline-flex items-center gap-1">
                        <Flame className="w-3.5 h-3.5 text-orange-300" /> {t('stats.totalXP')}
                    </p>
                ...
                <Card className="p-5">
                    <p className="text-sm text-gray-400 mb-1 inline-flex items-center gap-1">
                        <TrendingUp className="w-3.5 h-3.5 text-emerald-300" /> {t('stats.currentStreak')}
                    </p>
                    <p className="text-3xl font-bold text-orange-300">
                        5<span className="text-lg text-gray-500"> {t('common.days')}</span>
                    </p>
                </Card>
            </div>
            ...
            {/* Study Categories */}
            <div className="mb-8">
                <h2 className="text-2xl font-bold mb-4">{t('sections.study')}</h2>
            ...
            {/* Practice Categories */}
            <div>
                <h2 className="text-2xl font-bold mb-4">{t('sections.practice')}</h2>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
